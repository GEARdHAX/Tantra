...
const Dashboard = () => {
  ...
  const handleRelayToggle = (zoneId) => {
    ...
    // Note: This will send a command for 'mainTank' as well if you toggle it.
    // Your hardware code should be ready to handle or ignore this ID.
    sendCommand({
  ...
  const criticalPestZones = zones.filter(zone => zone.pestRisk === 'high').length;

  const getStatusColor = (zone) => {
    if (!zone || (zone.moisture === 0 && zone.temperature === 0)) return 'bg-gray-400';
  ...
  const formatTime = (timestamp) => timestamp ? new Date(timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : 'Never';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-gray-600">Real-time monitoring and control system</p>
        </div>
        <div className="flex items-center space-x-4">
          <LanguageSelector /> <TestVoiceButton />
        </div>
      </div>

      {/* Summary metrics cards (driven by Zone 1 real data) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500"><div className="flex items-center"><DropletIcon size={24} className="text-blue-500 mr-2" /><h3 className="text-sm font-medium text-gray-500">Avg Moisture</h3></div><p className="text-2xl font-bold text-gray-900 mt-2">{avgMoisture.toFixed(1)}%</p></div>
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-orange-500"><div className="flex items-center"><ThermometerIcon size={24} className="text-orange-500 mr-2" /><h3 className="text-sm font-medium text-gray-500">Avg Temperature</h3></div><p className="text-2xl font-bold text-gray-900 mt-2">{avgTemp.toFixed(1)}°C</p></div>
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-cyan-500"><div className="flex items-center"><Barrel size={24} className="text-cyan-500 mr-2" /><h3 className="text-sm font-medium text-gray-500">Tank Level</h3></div><p className="text-2xl font-bold text-gray-900 mt-2">{tankLevel}%</p></div>
        <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-red-500"><div className="flex items-center"><BugIcon size={24} className="text-red-500 mr-2" /><h3 className="text-sm font-medium text-gray-500">Pest Alerts</h3></div><p className="text-2xl font-bold text-gray-900 mt-2">{criticalPestZones}</p></div>
      </div>
                  ...
                  <div key={zone.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center">
                        ...
                        {(zone.id === 1 || zone.id === "zone1") && (
                          <span className="ml-3 text-xs font-mono bg-yellow-200 text-yellow-800 px-2 py-0.5 rounded-full">⚡ {generatedPower.toFixed(1)} W</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex space-x-2">
                      <button onClick={() => handleRelayToggle(zone.id)} className={`flex-1 flex items-center justify-center px-3 py-2 text-xs font-medium rounded-md transition-colors ${relayStates[zone.id] ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                        <PowerIcon size={14} className="mr-1" /> RELAY {relayStates[zone.id] ? 'ON' : 'OFF'}
                      </button>
                      <button className="flex-1 flex items-center justify-center px-3 py-2 text-xs font-medium rounded-md bg-gray-200 text-gray-400 cursor-not-allowed">
                        <SprayCanIcon size={14} className="mr-1" /> SPRAYER
                      </button>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center">
                  <Barrel size={20} className="text-cyan-500 mr-3" />
                  <div>
                    <p className="font-medium text-gray-900">Main Tank Relay</p>
                    <p className="text-sm text-gray-500">Water supply system</p>
                  </div>
                </div>
                <button onClick={() => handleRelayToggle('mainTank')} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${relayStates.mainTank ? 'bg-blue-600' : 'bg-gray-300'}`}>
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${relayStates.mainTank ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
           ...
           <div className="bg-white rounded-lg shadow-md overflow-hidden">
             ...
             <div className="p-6 space-y-2">
               {displayZones.map((zone) => (
                 <button key={zone.id} className="w-full flex items-center justify-between p-3 text-left hover:bg-gray-50 rounded-lg border">
                   <div className="flex items-center">
                     <div className={`w-2 h-2 rounded-full mr-3 ${getStatusColor(zone)}`}></div>
                     <span className="font-medium">{zone.name}</span>
                   </div>
                   <span className="text-sm text-gray-500">{zone.alerts?.length > 0 ? `${zone.alerts.length} alerts` : 'Normal'}</span>
                 </button>
               ))}
             </div>
           </div>
